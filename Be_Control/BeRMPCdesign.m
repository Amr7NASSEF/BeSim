...
if nargin < 2
   ...
   RMPCParam.Qu = 1e0*eye(model.pred.nu);
    ...
    nd = model.pred.nd;
    nu = model.pred.nu;
    nw = nd/4;%L2020 no of uncertain disturbance could be same as no Of know disturbance nd 
        ...
        if RMPCParam.Condensing %  Single shooting
            if k == 1
                ...
                %con = con + [ (y(:, k) == model.pred.Cd*x(:, k) + model.pred.Dd*uk + model.pred.Fd*1):['SSM_single_shoot_k=',int2str(k)] ];
                 %con = con + [ (
                 y(:, k) = model.pred.Cd*x(:, k) + model.pred.Dd*V(:,k) + model.pred.Fd*1;%):['SSM_single_shoot_k=',int2str(k)] ];% L2020 using V instead of U 
            elseif k==2
...
%                                                                  AW(:, (N-k+1)*nu+1 : end ) * reshape( W(:,1:k-1) , nr * (k-1) , 1) + ...
%                                                                  AG(:, (N-k+1)*1+1 : end ) * ones(k-1,1) ) + ...
%                                                                  model.pred.Dd*uk  + model.pred.Fd*1):['SSM_single_shoot_k=',int2str(k)] ];
                %con = con + [ 
                y(:, k) = model.pred.Cd*( AExpX0 + AB(:, (N-k+1)*nu+1 : end ) * reshape( V(:,1:k-1) , nu * (k-1) , 1) + ...
                                                                  AE(:, (N-k+1)*nd+1 : end ) * reshape( d_prev(:,1:k-1) , nd * (k-1) , 1) + ...
                                                                  AW(:, (N-k+1)*nw+1 : end ) * reshape( W(:,1:k-1) , nw * (k-1) , 1)+ ...
                                                                  AG(:, (N-k+1)*1+1 : end ) * ones(k-1,1) ) + ...
                                                                  model.pred.Dd*  (V(:,k)+  kron((tril(ones(k-1))),Lxx)*reshape( W(:,1:k-1) , nw * (k-1) , 1))...
                                                                  + model.pred.Fd*1;%:['SSM_single_shoot_k=',int2str(k)]];
...
%                                                                  model.pred.Dd*uk  + model.pred.Fd*1):['SSM_single_shoot_k=',int2str(k)] ];
                ...
                y(:, k) = model.pred.Cd*( AExpX0 + AB(:, (N-k+1)*nu+1 : end ) * reshape( V(:,1:k-1) , nu * (k-1) , 1) + ...
                                                                  ...
                                                                  AG(:, (N-k+1)*1+1 : end )  * ones(k-1,1) ) + ...
                                                                  model.pred.Dd* (V(:,k) +  kron(([1:k-1]),Lxx)*reshape( W(:,1:k-1) , nw * (k-1) , 1) )+ ...
                                                                  model.pred.Fd*1;%):['SSM_single_shoot_k=',int2str(k)]];
        ...
        %   input constraints
         ...
         con = con + [ (model.pred.umin <= (V(:,k) +  kron(([1:k-1]),Lxx)*reshape( W(:,1:k-1) , nw * (k-1) , 1) )  <= model.pred.umax):['u_box_k=',int2str(k)] ];
    ...
    %   -------------  OBJECTIVE FUNCTION  -------------
        %    % quadratic objective function withouth states constr.  penalisation
                ...
                obj = obj + s(:,k)'*Qsb*s(:,k) + ...         %  comfort zone penalization
                   P*((V(:,k) +  kron(([1:k-1]),Lxx)*reshape(W(:,1:k-1), nw*(k-1) , 1) )'*Qu*(V(:,k) +  kron(([1:k-1]),Lxx)*reshape( W(:,1:k-1) , nw * (k-1) , 1) ));  
