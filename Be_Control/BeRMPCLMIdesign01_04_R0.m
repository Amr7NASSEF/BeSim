...
%BeMPCdesign(model, RMPCLMIParam)
...
% comfort zone and price preview 
 ...
 if k > Nrp
    ...
    Lmi_rie =[[1, x'; x, W >= ZERO] :['Lmi_rie_k=',int2str(k)] ];% condition to minimize gamma
...
% LMI for convix
    ...
    for v = 1 : nv
      lmi_conv_item = [ 
        ...
        sqrt(Qy)*Y     , ZEROux             ,ZEROux        , Gamma*Iu ] >= ZERO :['Lmi_conex_k=',int2str(k)] ];
...
% LMI for input       
    ...
    if(isempty(model.pred.umax) == 0)
        ...
        Lmi_u_max = [[ [ diag(model.pred.umax.^2), Y(:,1+(k-1)*nx:nx+(k-1)*nx);...
            Y(:,1+(k-1)*nx:nx+(k-1)*nx)', W(:,1+(k-1)*nx:nx+(k-1)*nx)] >= ZERO]:['L2_U_k=',int2str(k)] ];

        % L1-norm
%         Lmi_u_max = Lmi_u_max + [[ [ U_Cons(:,1+(k-1)*nu:nu+(k-1)*nu), Y(:,1+(k-1)*nx:nx+(k-1)*nx);...
%             Y(:,1+(k-1)*nx:nx+(k-1)*nx)', W(:,1+(k-1)*nx:nx+(k-1)*nx)] >= ZERO]:['L1_U_k=',int2str(k)] ];
%         for j = 1 : nu
%             Lmi_u_max = Lmi_u_max + [ [U_Cons(j,j+(k-1)*nu) <= (model.pred.umax(j))^2]:['LMI_U_k=',int2str(k)] ];
%         end 
    end 
        
    wm=[350;350;350;350;350;350];% Slack variable to be used% and % ymax 
% LMI for output
    Lmi_output_max = [];
    if(isempty(wa) == 0)
        ...
        for v = 1 : nv
            ...
            %%% 
        lmi_output_max_item = [ 
            [diag((wm.^2)+s(:,k)) , C{v}*(A{v}*W + B{v}*Y);...
            (A{v}*W + B{v}*Y)'*C{v}', W  >= ZERO]:['Lmi_Y_k=',int2str(k)] ];% adding slack variable to the output y 
        
%         lmi_output_max_item = [ 
%             [W(:,1+(k-1)*nx:nx+(k-1)*nx) ,(A{v}*W(:,1+(k-1)*nx:nx+(k-1)*nx) + B{v}*Y(:,1+(k-1)*nx:nx+(k-1)*nx))'*C{v}';...
%              C{v}*(A{v}*W(:,1+(k-1)*nx:nx+(k-1)*nx) + B{v}*Y(:,1+(k-1)*nx:nx+(k-1)*nx)),  diag((wm.^2)+s(:,k)) ] >= ZERO ];% adding slack variable to the output y 
%         
