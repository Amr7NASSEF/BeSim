   ...
   %% Input Formulation 
    ...
    %   -------------  Constraints  -------------
        ...
        if RMPCParam.Condensing %  Single shooting
            if k == 1
                ...
                AW(:, (N-k)*nw+1:(N-k+1)*nw) = 1*ones(nx,nw);        % G matrix in paper  the impact of Uncertainty on States dynamics 
    ...
    %   -------------  OBJECTIVE FUNCTION  -------------
        %    % quadratic objective function withouth states constr.  penalisation
                    ...
                    obj = obj + norm(s(:,k),1) + norm(u(:,k),1) ;  %+ 0.01 * norm(sH(:,k),1) + 0.01 * norm(sL(:,k),1);%+ 1e-3*(norm(sL(:,k)) + norm(sH(:,k))) + 1e-18 * norm(V(:,k)); %+ norm( max(LM)); %+ max (norm(LM + norm(V(:,k),inf) ;% + norm(uk,Inf); ...         %  comfort zone penalization
                              %P*(V(:,k)'*Qu*V(:,k)); 
               if k>=2
