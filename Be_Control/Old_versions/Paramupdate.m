...
if not(MHEParam.Condensing)
    Ex = sdpvar(nx, N, 'full'); %  state update error slack
...
for k = 1:N   
       ...
       %     state + output update equations
        if MHEParam.Condensing 
            if k == 1
                ...
                con = con + [ y(:, k) == model.pred.Cd*( AExpX0 + AB(:, (N-k+1)*nu+1 : end ) * reshape( u(:,1:k-1) , nu * (k-1) , 1) + ...
                                                                 ...
                                                                 AG(:, (N-k+1)*1+1 : end ) * ones(k-1,1) + Aresd*AExpX0 +Bresd *u(:,k) + Ex(:,k) ) + ...
            ...
            con = con + [ x(:, k+1) == model.pred.Ad*x(:, k) + model.pred.Bd*u(:, k) + model.pred.Ed*d(:, k) + model.pred.Gd*1 + Aresd*AExpX0 +Bresd *u(:,k) + Ex(:, k)];
            con = con + [ y(:, k) == model.pred.Cd*x(:, k)  + model.pred.Dd*u(:, k)  + model.pred.Fd*1 + v(:, k)];
        end
        ...
        if not(MHEParam.Condensing)        
           ...
           con = con + [  -30*ones(nx,1) <= Ex(:, k) <= 30*ones(nx,1)];   
        elseif k == 1
        ...
        %   -------------  OBJECTIVE FUNCTION  -------------
         if MHEParam.Condensing 
             ...
             obj = obj +  Ex(:,k)'*MHEParam.Re*Ex(:,k);
         else
             obj = obj + Ex(:,k)'*MHEParam.Qe*Ex(:,k) + ...         %  state update error penalization
...
if MHEParam.Condensing 
   mhe = optimizer(con, obj, options,  {y, u, d, xest}, {Aresd; Bresd; obj; Ex } );  
else
   mhe = optimizer(con, obj, options,  {y, u, d, xest}, {Aresd; Aresd; obj; Ex } ); 
end
