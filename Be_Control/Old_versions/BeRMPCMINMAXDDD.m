function [mpc, constraints_info] = BeRMPCMINMAX(model, RMPCParam)
   ...
   %% Input Formulation 
    ...
    %   -------------  OBJECTIVE FUNCTION  -------------
        %    % quadratic objective function withouth states constr.  penalisation
         ...
         obj = obj + norm(s(:,k),1) + 1000* norm((y(:,k)-(wb+1)),1);%+ 1 * norm((y(:,k)-wa),1);% just the slack?? 1000
%          obj = obj + norm(s(:,k),1) + 50 * norm((y(1,k)-wb),1)+...
%              50 * norm((y(2,k)-wb),1)+ 0.1 * norm((y(3,k)-wb),1)+...
%              0.01 * norm((y(4,k)-wb),1)+ 0.01 * norm((y(5,k)-wb),1)+ 0.01 * norm((y(6,k)-wb),1);%+ 0.01 * norm((y(:,k)-wa),1);% just the slack??
