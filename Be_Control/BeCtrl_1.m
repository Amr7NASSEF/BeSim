...
if CtrlParam.RMPCLMI.use
    ...
    if  strcmp(model_11.buildingType,'HollandschHuys')
        % weight diagonal matrices
        controller.RMPCLMI.Qw = model.pred.Cd'*model.pred.Cd{1,1}*10^5;
        controller.RMPCLMI.Qy = eye(nu)*10^-3;
    else
        % weight diagonal matrices
        controller.RMPCLMI.Qw = model.pred.Cd'*model.pred.Cd{1,1}*10^5;
        controller.RMPCLMI.Qy = eye(nu)*10^-3;
    ...
    [controller.RMPCLMI.optimizer, controller.RMPCLMI.constraints_info] = BeRMPCLMI(model_11,model_22,model_33,controller.RMPCLMI);
    fprintf('*** Done.\n')
