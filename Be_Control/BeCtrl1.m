...
if  strcmp(model.buildingType,'HollandschHuys')    
    ...
    controller.RMPC.Qu = 1e0*eye(model.pred.nu);
     ...
     % horizons
    controller.RMPC.N = 22;
    controller.RMPC.Nc = 22;
    controller.RMPC.Nrp = 22;
    controller.RMPC.Ndp = 22;
    % weight diagonal matrices 
    controller.RMPC.Qsb = 1e4*eye(model.pred.ny);
    controller.RMPC.Qsa = 1e4*eye(model.pred.ny);
    controller.RMPC.Qu = 1e-02*eye(model.pred.nu);
    controller.RMPC.Qx = 1*eye(model.pred.nx);
end   
    %  MPC optimizer synthesis   
    [controller.RMPC.optimizer, controller.RMPC.constraints_info] = BeRMPCLMIdesign(model, controller.RMPC);
    fprintf('*** Done.\n')
