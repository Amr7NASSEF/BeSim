...
%             u_traj(:,k) = u(:,k);
        ...
        if RMPCParam.Condensing %  Single shooting
            if k == 1
                ...
                AW(:, (N-k)*nr+1:(N-k+1)*nr) = eye(nx,nr)*0.4;        % G matrix to connect 
...
%                                                                  model.pred.Dd*uk  + model.pred.Fd*1):['SSM_single_shoot_k=',int2str(k)] ];
                ...
                AW(:, (N-k)*nr+1:(N-k+1)*nr ) = model.pred.Ad* AW(:, (N-k+1)*nr+1:(N-k+2)*nr );
                if k==N-1
                     AW(:, (N-k)*nr+1:(N-k+1)*nr ) = model.pred.Ad* 0 *AW(:, (N-k+1)*nr+1:(N-k+2)*nr );
                end
            end  
