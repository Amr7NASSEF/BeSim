function [mpc, constraints_info] = BeMPCdesign_w_U(model, RMPCParam)
...
if nargin < 2
  ...
  %% MARCH 2020
      ...
      %u = sdpvar(nu,N,'full');
%       Lxx = zeros(nu,nw); % mapping parameterisation of uncertain W on input U
%       Lxx(1)=1;
...
%     u_traj = sdpvar(nu, Nc, 'full');
    ...
    L  = zeros( nu , (N-1)*nw); % to be used to formulate input U= L*W +V, we use Lxx inside L 
    AExpX0 = eye(nx) * x(:,1);
   ...
   %% Input Formulation 
   if k==1
       L(:, (N-k)*nw+1:(N-k+1)*nw)=Lxx;
       u(:,k)=V(:,k); 
   else
       G = G +[u(:,k)==V(:,k)+L(:, (N-k+1)*nw+1 : end ) * reshape( W(:,1:k-1) , nw * (k-1) , 1)];
       L(:, (N-k)*nw+1:(N-k+1)*nw ) = Lxx;
   end
...
%             u_traj(:,k) = u(:,k);
        ...
        if RMPCParam.Condensing %  Single shooting
            if k == 1
                ...
                AW(:, (N-k)*nw+1:(N-k+1)*nw) = eye(nx,nw)*1;        % G matrix in paper  the impact of Uncertainty on States dynamics 
                
                %con = con + [ (y(:, k) == model.pred.Cd*x(:, k) + model.pred.Dd*uk + model.pred.Fd*1):['SSM_single_shoot_k=',int2str(k)] ];
                y(:, k) = model.pred.Cd*x(:, k) + model.pred.Dd*uk + model.pred.Fd*1;
            else
