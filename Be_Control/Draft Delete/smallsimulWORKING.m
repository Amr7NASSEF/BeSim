...
model3 = BeModel('RenoLight', ModelParam);      % construct a model object   
model2 = BeModel('Reno', ModelParam);      % construct a model object   
model1 = BeModel('Old', ModelParam);      % construct a model object   

model = model1;      % construct a model for prediction   

%% Disturbacnes 
% ambient temperature, solar radiation, internal heat gains
DistParam.reload = 0;
dist = BeDist(model, DistParam);
%mpc=BeRMPCLMI(model_Light,model_Reno,model_Old);
...
% LMI for output
     ...
     for v = 1 : nv
         lmi_output_max_item = [[
             [ W ,   (A{v}*W + B{v}*Y)'*C{v}' ;...
             eye(ny)*(C{v}*(A{v}*W + B{v}*Y)), diag(ymax) * diag(ymax) + diag(s)*10^5 ] >= 0] :['Lmi_maximum output constraint']];  
